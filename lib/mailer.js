...
export async function sendResetOtpEmail(toEmail, otpCode, siteName) {
  const transporter = createTransporter();
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:Arial,sans-serif; max-width:480px; margin:0 auto;">
        <h2 style="color:#1565c0;">${brand}</h2>
        <p>Kami menerima permintaan reset password untuk akun kamu. Gunakan kode berikut:</p>
        <div style="font-size:32px; font-weight:800; letter-spacing:8px; background:#f2f5f9; color:#1565c0; padding:16px 20px; border-radius:10px; text-align:center; margin:16px 0;">${otpCode}</div>
        <p style="color:#555; font-size:13px;">Kode berlaku <b>10 menit</b>. Jangan bagikan kode ini ke siapa pun, termasuk pihak yang mengaku dari ${brand}.</p>
        <p style="color:#999; font-size:12px;">Kalau kamu tidak meminta reset password, abaikan email ini -- password kamu tetap aman.</p>
      </div>
    `
  });
}
