...
// Update HARGA banyak produk sekaligus (digiflazzBasePrice + price beda-beda per produk, BUKAN
// patch yang sama kayak updateProductsByIds di atas) dalam SATU baca + SATU tulis file. Dipakai
// khusus buat sinkron harga massal (lib/digiflazz.js syncAllDigiflazzPrices) yang bisa nyentuh
// ratusan/ribuan produk Digiflazz sekaligus.
//
// ALASANNYA: updateProduct() biasa baca ULANG SELURUH file products.json tiap 1x dipanggil
// (getAllProducts() di awal fungsinya), lalu tulis ULANG SELURUH file lagi tiap 1x selesai. Kalau
// dipanggil di dalam loop buat N produk sekaligus (kayak sebelumnya di syncAllDigiflazzPrices),
// itu jadi N kali baca + N kali tulis file yang sama, SECARA SINKRON (fs.writeFileSync/renameSync
// di db.js -- Node single-threaded, jadi ini BLOCKING event loop). Buat toko dengan ratusan/ribuan
// produk Digiflazz, itu bisa bikin proses Node "macet" sesaat SETIAP KALI auto-sync jalan (tiap 30
// menit) -- SEMUA user lain yang lagi buka situs bersamaan ikut kena lag di jendela waktu itu,
// bukan cuma bikin sinkronnya sendiri lambat.
//
// Fungsi ini baca file SEKALI, terapkan SEMUA perubahan harga di memory, lalu tulis SEKALI --
// jadi biaya baca+tulisnya TETAP (gak ngikutin jumlah produk yang disinkron), walau yang
// disinkron ratusan/ribuan produk sekaligus. Ini kenapa auto-sync gak perlu dikasih jeda/delay
// buatan kayak batch import/hapus massal di admin (yang jedanya buat alasan LAIN: hindari
// "Payload Too Large" & request HTTP yang nge-block lama -- gak relevan di sini karena job ini
// jalan sendiri di server, bukan nunggu 1 request HTTP dari admin).
export function updateProductPrices(updates) {
  // updates: array of { id, digiflazzBasePrice, price }
  if (!Array.isArray(updates) || updates.length === 0) return 0;
  const patchMap = new Map(updates.map(u => [u.id, u]));
  const products = getAllProducts();
  let count = 0;
  for (let i = 0; i < products.length; i++) {
    const patch = patchMap.get(products[i].id);
    if (!patch) continue;
    products[i] = {
      ...products[i],
      digiflazzBasePrice: Number(patch.digiflazzBasePrice) || 0,
      price: Number(patch.price) || 0
    };
    count++;
  }
  if (count > 0) writeDB('products', products);
  return count;
}
