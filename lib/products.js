...
export function createProduct({ name, category, description, price, stockNote, thumbnail, stockItems, gamePreset, customTargetFields, provider, digiflazzSku, digiflazzCustomerNoTemplate, variantGroup, digiflazzBasePrice, marginType, marginValue, costPrice }) {
  ...
  const product = {
    ...
    digiflazzBasePrice: provider === 'digiflazz' ? (Number(digiflazzBasePrice) || 0) : 0,
    variantGroup: String(variantGroup || '').trim(),
    marginType: provider === 'digiflazz' && (marginType === 'percent' || marginType === 'fixed') ? marginType : '',
    ...
    marginValue: provider === 'digiflazz' && marginValue !== undefined && marginValue !== '' && marginValue !== null ? Number(marginValue) : null,
    costPrice: provider === 'digiflazz' ? 0 : (Number(costPrice) || 0), // harga modal Stok Manual (opsional), buat hitung Pendapatan Bersih
    status: 'active',
