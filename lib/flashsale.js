...
// Kebalikan recordFlashSaleSale -- dipanggil kalau order yang SEBELUMNYA sempat kehitung
// soldCount (dianggap laku, quota kepakai) belakangan ternyata GAGAL (mis. order Digiflazz yang
// awalnya "Pending" lalu dicek ulang job background ternyata "Gagal"). Tanpa ini, quota Flash
// Sale bisa keliru kepotong/habis duluan gara-gara transaksi yang sebenarnya gak jadi. Diklem
// minimal 0 biar gak pernah negatif walau dipanggil dobel/gak sinkron.
export function reverseFlashSaleSale(productId, qty = 1) {
  const items = getAllFlashSaleItems();
  const item = items.find(it => it.productId === productId);
  if (!item) return;
  item.soldCount = Math.max(0, (item.soldCount || 0) - Math.max(1, Number(qty) || 1));
  saveFlashSaleItems(items);
}
  ...
  return items
    .map(item => {
      ...
      return {
        ...
        // BUG: sebelumnya `product.variantGroup || product.name` -- variantGroup itu cuma nama
        // brand/grup buat pengelompokan varian di katalog (mis. "Free Fire" doang), BUKAN nama
        // produk lengkap (mis. "Free Fire 140 Diamond"). Karena hampir semua produk auto (Digiflazz)
        // punya variantGroup keisi, flash sale-nya selalu nampilin nama brand doang, bukan nominal
        // spesifiknya -- padahal customer perlu tau PERSIS nominal mana yang lagi promo.
        name: product.name,
...
// Versi "batch" getEffectivePrice buat halaman yang ngitung harga BANYAK produk sekaligus (katalog
// /produk, daftar varian nominal di halaman detail produk). getActiveFlashPriceForProduct() di atas
// baca config.json + flashsale.json dari disk SETIAP kali dipanggil -- kalau dipanggil di dalam
// loop per produk/varian (bisa belasan-puluhan dalam 1 halaman), 2 file itu ke-baca ulang berkali-
// kali padahal isinya sama persis sepanjang request yang sama. Ini salah satu penyebab utama
// halaman produk/detail kerasa lag pas produknya/variannya banyak.
//
// Pemakaian: `const resolvePrice = createPriceResolver(user);` sekali di awal handler, lalu panggil
// `resolvePrice(product)` per produk -- hasilnya SAMA PERSIS kayak manggil getEffectivePrice(product,
// user) satu-satu, cuma config/flashsale-nya dibaca sekali aja di awal, bukan diulang tiap produk.
export function createPriceResolver(user) {
  const running = isFlashSaleRunning();
  const flashItemByProductId = running
    ? new Map(getAllFlashSaleItems().filter(it => it.active !== false).map(it => [it.productId, it]))
    : new Map();

  return function resolvePrice(product) {
    const item = flashItemByProductId.get(product.id);
    if (item && !isFlashSaleQuotaReached(item)) return item.flashPrice;
    return user ? applyMemberDiscount(product.price, user.membership) : product.price;
  };
}
