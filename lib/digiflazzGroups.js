...
// Dipanggil pas seluruh Grup Varian dihapus (lihat deleteProductsByGroup di lib/products.js) --
// biar entry foto grupnya gak nyangkut jadi sampah di data/digiflazzGroups.json. Aman dipanggil
// walau grup itu gak pernah punya foto custom (gak ngapa2in, gak throw).
export function deleteGroupThumbnail(variantGroup) {
  const name = String(variantGroup || '').trim();
  if (!name) return;
  const rows = readDB('digiflazzGroups', []);
  const filtered = rows.filter(r => r.variantGroup !== name);
  if (filtered.length !== rows.length) writeDB('digiflazzGroups', filtered);
}
