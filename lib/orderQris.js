...
export async function createOrderQrisPayment({ user, product, qty, targetData, targetText, unitPrice, orderTotal }) {
  const cfg = getConfig();
  const qCfg = cfg.qris || {};
...
export async function checkPendingOrderQrisPayments() {
  ...
  for (const p of stillPending) {
    ...
    const paidRecord = getOrderQrisPayments();
    paidRecord[p.trxid].status = 'paid';
    paidRecord[p.trxid].paidAt = new Date().toISOString();
    await saveOrderQrisPayments(paidRecord);

    // Langsung bikin & proses order-nya DI SINI (server-side, gak butuh browser terbuka) --
    // ini inti bedanya dari alur lama. Kalau gagal, catat errornya tapi status QRIS TETAP
    // "paid" (uangnya sudah masuk) -- admin bisa lihat & follow up manual lewat orderIds
    // yang kosong sebagai sinyal "sudah bayar tapi order belum kebuat".
    try {
      ...
      if (user && product) {
        ...
        afterFulfill[p.trxid].orderIds = orders.map(o => o.id);
        await saveOrderQrisPayments(afterFulfill);
      } else {
        console.error(`[orderQris] User/produk tidak ditemukan buat trxid ${p.trxid} setelah bayar -- perlu ditindak manual.`);
      }
    } catch (fulfillErr) {
      console.error(`[orderQris] Gagal proses order setelah bayar (trxid ${p.trxid}):`, fulfillErr.message);
    }
  }
