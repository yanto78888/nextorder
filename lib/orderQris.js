...
async function markOrderQrisPaid(trxid) {
  ...
  try {
    ...
    if (user && product) {
      ...
      // Komisi referral -- sama kayak checkout saldo, diproses PER-ORDER (skema flat/persen beda
      // tergantung provider). Cuma order yang udah PASTI 'completed' di titik ini aja. Order yang
      // masih 'processing' (Digiflazz/IndoSMM) baru dikreditkan belakangan pas beneran selesai --
      // lihat catatan lengkap di routes/user.js.
