...
import { getProcessedMutations, saveProcessedMutations, getDeposits } from './deposit.js';
...
// Gabungan total SEMUA pending record yang lagi aktif (beli-produk-QRIS + deposit) -- dipakai
// getKodeUnik() (lib/qris.js) buat mastiin kodeUnik yang di-generate gak bikin nominal akhir
// bentrok sama pending record lain manapun. Lihat catatan lengkap di getKodeUnik().
function getActivePendingTotals() {
  const totals = new Set();
  Object.values(getOrderQrisPayments()).forEach(o => { if (o.status === 'pending') totals.add(o.total); });
  Object.values(getDeposits()).forEach(d => { if (d.status === 'pending') totals.add(d.total); });
  return totals;
}
...
export async function createOrderQrisPayment({ user, product, qty, targetData, targetText, unitPrice, orderTotal, promoCode = '' }) {
  ...
  const baseTotal = orderTotal + fee;
  const kodeUnik = getKodeUnik(baseTotal, getActivePendingTotals());
  const total = baseTotal + kodeUnik;
...
async function markOrderQrisPaid(trxid) {
  ...
  try {
    ...
    if (user && product) {
      ...
      // Komisi referral -- sama kayak checkout saldo, dihitung dari total order yang BENERAN
      // sukses aja, dan baru dikreditkan di sini (bukan pas /order/qris-init) karena QRIS-nya
      // baru beneran lunas & order baru beneran jadi di titik ini. Satu persentase SAMA RATA
      // berlaku ke SEMUA jenis produk & metode bayar, lihat creditReferralCommission().
