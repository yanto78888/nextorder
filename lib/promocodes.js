...
// =====================================================================
// KODE PROMO
// Diskon PERSENTASE (bukan potongan nominal tetap) yang bisa dibuat & dihapus admin,
// dengan batas TOTAL berapa kali kode ini boleh kepakai (maxUses, gabungan semua user)
// dan tiap user CUMA BOLEH pakai kode yang sama SATU KALI (gak bisa dobel) -- dicek dari
// riwayat redemptions, bukan dibatasi per sesi/browser (biar gak bisa dikelabui logout-login
// akun baru pakai email lain, tapi TETAP kepakai berkali-kali kalau memang user berbeda beneran,
// sesuai kuota maxUses-nya).
//
...
export function createPromoCode({ code, discountPercent, maxUses }) {
  ...
  if (!Number.isFinite(percent) || percent < 1 || percent > 100) {
    throw new Error('Diskon harus angka 1-100 (persen)');
  }
...
// dua request itu). Fungsi ini SENGAJA read-only (gak nambah usedCount) -- itu tugas
// redeemPromoCode() yang dipanggil TERPISAH setelah order-nya beneran sukses.
export function validatePromoForUser(code, userId) {
...
// amount = TOTAL belanja (bukan harga per unit) -- pembulatan pakai Math.round di sini SATU
// tempat, dipakai sama-sama oleh preview harga (routes/user.js) dan checkout beneran, biar
// angka diskon yang ditampilkan ke user PERSIS SAMA dengan yang benar-benar dipotong.
