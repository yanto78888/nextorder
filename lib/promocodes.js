...
// =====================================================================
// KODE PROMO
// Diskon PERSENTASE (mis. potong 10% dari total belanja) dengan syarat MINIMAL BELANJA opsional
// (mis. min. Rp 50.000), dibuat & dihapus admin, dengan batas TOTAL berapa kali kode ini boleh
// kepakai (maxUses, gabungan semua user) dan tiap user CUMA BOLEH pakai kode yang sama SATU KALI
// (gak bisa dobel) -- dicek dari riwayat redemptions, bukan dibatasi per sesi/browser (biar gak
// bisa dikelabui logout-login akun baru pakai email lain, tapi TETAP kepakai berkali-kali kalau
// memang user berbeda beneran, sesuai kuota maxUses-nya).
//
...
export function createPromoCode({ code, discountPercent, minPurchase, maxUses }) {
  ...
  if (!Number.isFinite(percent) || percent < 1 || percent > 100) {
    throw new Error('Persentase diskon harus antara 1-100');
  }
  ...
  const item = {
    id: genId('PROMO'),
    code: cleanCode,
    discountPercent: percent, // potongan PERSEN dari total belanja, bukan nominal tetap
...
// amount = TOTAL belanja (bukan harga per unit) -- dipakai sama-sama oleh preview harga
// (routes/user.js) dan checkout beneran, biar angka diskon yang ditampilkan ke user PERSIS SAMA
// dengan yang benar-benar dipotong. Math.min di akhir cuma jaga-jaga (harusnya gak akan pernah
// kepakai selama discountPercent divalidasi <=100 di createPromoCode), biar potongannya gak
// mungkin lebih besar dari total-nya sendiri walau apapun yang terjadi.
export function computePromoDiscount(promo, amount) {
  if (!promo) return 0;
  return Math.min(amount, Math.round(amount * promo.discountPercent / 100));
}
