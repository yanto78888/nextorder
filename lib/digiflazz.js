  ...
  for (const order of pendingOrders) {
    ...
    try {
      ...
      if (status === 'sukses') {
        ...
        // Order ini sempat dihitung "terjual" & (kalau relevan) makan quota Flash Sale waktu masih
        // berstatus "Pending" (lihat catatan refund di routes/user.js: baru dianggap gagal & di-skip
        // begitu Digiflazz balikin status Gagal LANGSUNG, bukan pas masih Pending). Angka "Terjual"
        // sendiri sekarang dihitung live dari order.status (bukan counter manual) jadi otomatis benar
        // begitu status order di atas jadi 'cancelled' -- gak perlu di-patch lagi di sini. Tapi
        // soldCount Flash Sale MASIH counter manual, jadi tetap perlu dibalikin manual kalau order ini
        // tadinya kepakai harga flash.
