...
// Order-order (bisa lebih dari 1, lihat catatan iterations di orderEngine.js) yang lahir dari
// SATU request API reseller dengan ref_id tertentu -- dipakai routes/api.js buat idempotency:
// kalau reseller nge-retry request yang sama (mis. karena timeout di sisi mereka padahal
// request pertama sebenarnya sudah diproses), request kedua gak bikin order/charge baru lagi,
// cukup dibalikin hasil yang SAMA kayak request pertama.
export function findOrdersByApiRefId(userId, apiRefId) {
  if (!apiRefId) return [];
  return getAllOrders().filter(o => o.userId === userId && o.apiRefId === apiRefId);
}
...
export function createOrder({
  ...
  providerCustomerNo = '',
  indosmmServiceId = '', // SNAPSHOT service id IndoSMM SAAT order dibuat (bukan diambil ulang dari
                          // produk belakangan) -- produk bisa dihapus admin atau indosmmServiceId-nya
                          // diganti ke layanan lain di kemudian hari, tapi flag "bisa dibatalkan/direfill"
                          // punya ORDER ini harus tetap berdasarkan layanan yang BENERAN dipakai waktu
                          // order ini dibuat, bukan ikut berubah-ubah mengikuti kondisi produk saat ini.
  ...
  apiRefId = '' // ref_id kiriman reseller (kalau order ini lahir dari API, lihat routes/api.js) --
                // dipakai findOrdersByApiRefId buat idempotency, kosong kalau order dari checkout web biasa.
}) {
  ...
  const order = {
    ...
    providerRefId,
    providerCustomerNo,
    indosmmServiceId: provider === 'indosmm' ? String(indosmmServiceId || '') : '',
...
// Ranking "Top N produk terlaris" (by qty terjual) buat widget "Top Produk Populer" di dashboard
// admin. Prinsipnya SAMA kayak getTotalSoldMap() di atas (dihitung live dari order asli tiap
// dipanggil, order 'cancelled' TIDAK dihitung -- lihat alasan lengkapnya di komentar
// getTotalSoldMap), bedanya cuma sekalian ranking + total omzet per produk dalam 1x jalan, biar
// caller (routes/admin.js) gak perlu scan ulang seluruh order lagi buat hitung omzet per produk.
export function getTopSellingProducts(limit = 10) {
  const orders = getAllOrders();
  const map = {}; // productId -> { qty, revenue }
  orders.forEach(o => {
    if (o.status === 'cancelled') return;
    if (!o.productId) return;
    if (!map[o.productId]) map[o.productId] = { qty: 0, revenue: 0 };
    map[o.productId].qty += Number(o.qty) || 0;
    map[o.productId].revenue += Number(o.total) || 0;
  });
...
// Rekap transaksi HARIAN buat grafik di halaman PUBLIK (Live Transaksi) -- cuma jumlah order &
// total omzet (TANPA harga modal/margin, itu data sensitif bisnis) per hari. Sengaja dipisah dari
// getMonthlyRevenueStats (dipakai admin, bawa data net/modal) supaya gak ada jalur yang gak
// sengaja mengekspos angka modal ke endpoint publik.
export function getPublicDailyStats(days = 14) {
  ...
  orders.forEach(o => {
    ...
    if (!bucket) return; // di luar jendela `days` hari, dilewati
    bucket.orders += 1;
    bucket.total += Number(o.total) || 0;
  });
  return buckets.map(b => ({ label: b.label, orders: b.orders, total: Math.round(b.total) }));
}

// Sama kayak getPublicDailyStats tapi per BULAN (buat toggle "Bulanan" di grafik publik) --
// polanya sama persis kayak getMonthlyRevenueStats di bawah, cuma gak nyertain field `net`.
export function getPublicMonthlyStats(monthsCount = 6) {
  const orders = getAllOrders().filter(o => o.status !== 'cancelled');
  const now = new Date();
  const buckets = [];
