...
export async function notifyOrder({
  ...
  qty = 1, unitPrice = null, apiRefId = '', paymentMethod = '', orderStatus = ''
}) {
  const cfg = getConfig();
  ...
  const tag = source === 'admin'
    ...
    : source === 'auto'
      ...
      : source === 'api'
        ...
        : '🛒 ORDER BARU';
  // Label status order dalam Bahasa Indonesia yang gampang dipindai admin di chat Telegram --
  // orderStatus di sini nilai mentah dari kolom order.status ('completed'/'processing'/'cancelled').
  const statusLabelMap = {
    completed: '✅ Selesai',
    processing: '⏳ Diproses',
    cancelled: '❌ Dibatalkan (saldo dikembalikan)'
  };
  const statusLabel = statusLabelMap[orderStatus] || '';
  await sendTelegramMessage(
    `${tag}\n\n` +
    `User: <b>${escapeHtml(username)}</b>\n` +
    ...
    (paymentMethod ? `Metode Bayar: <b>${escapeHtml(paymentMethod)}</b>\n` : '') +
