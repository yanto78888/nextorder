...
// Batas qty buat produk provider MANUAL/stok sistem (bukan Digiflazz/IndoSMM yang sudah punya
// batasnya sendiri di atas/di bawah). Sebelumnya provider manual SAMA SEKALI gak divalidasi di
// sini -- attribute `max` di <input type="number"> pada produk-detail.ejs cuma batas TAMPILAN
// (gampang dilewatin lewat submit form manual / curl langsung ke POST /order atau /api/order),
// jadi qty berapa pun (misal 999999) bisa lolos checkout selama saldo user cukup. Order sebesar
// itu tetap "sah" secara saldo, tapi jadi antrean kirim manual admin yang gak wajar & gak
// sengaja gampang ke-trigger salah klik. Angka 99 di sini SAMA PERSIS dengan batas atas atribut
// `max` yang dipakai di sisi tampilan (lihat qEl.max di produk-detail.ejs), biar batas tampilan
// & batas server konsisten -- bukan aturan baru, cuma nutup celah validasi yang ketinggalan.
export const MAX_MANUAL_QTY_PER_ORDER = 99;
...
export function validateQty(product, qty) {
  ...
  } else if (product.provider !== 'digiflazz' && qty > MAX_MANUAL_QTY_PER_ORDER) {
    return `Maksimal ${MAX_MANUAL_QTY_PER_ORDER}x per transaksi untuk produk ini. Silakan checkout terpisah untuk jumlah lebih banyak.`;
  }
  return null;
}
