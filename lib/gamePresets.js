...
export const GAME_PRESETS = {
  ...
  phone_number: {
    key: 'phone_number',
    label: 'Nomor HP (Pulsa/Data)',
    icon: 'fa-phone',
    fields: [
      { key: 'phoneNumber', label: 'Nomor HP', placeholder: 'Contoh: 081234567890', type: 'tel', required: true }
    ]
  },
  email: {
    key: 'email',
    ...
    icon: 'fa-envelope',
    fields: [
      { key: 'email', label: 'Email', placeholder: 'Contoh: nama@email.com', type: 'email', required: true }
    ]
  },
  custom: {
    key: 'custom',
...
export function formatGameTarget(gamePresetKey, inputs = {}) {
  switch (gamePresetKey) {
    ...
    case 'phone_number':
      return inputs.phoneNumber ? String(inputs.phoneNumber).trim() : '';

    case 'email':
      return inputs.email ? String(inputs.email).trim() : '';
