...
// Format: "nxo_" + 40 karakter hex acak, biar gampang dikenalin sekilas ini API key Nextorder
// (mirip konvensi provider lain kayak "sk_..." Stripe) dan gampang dibedain dari id/token lain.
export function findUserByApiKey(apiKey) {
  if (!apiKey) return null;
  return getAllUsers().find(u => u.apiKey && u.apiKey === apiKey) || null;
}

// Bikin API key baru buat user (dipanggil pas pertama kali generate ATAU regenerate/rotate).
// Regenerate LANGSUNG bikin key lama gak berlaku lagi (cukup ganti field-nya, key lama gak
// disimpan di mana-mana lain jadi otomatis invalid begitu diganti).
export function generateApiKey(id) {
  const apiKey = 'nxo_' + crypto.randomBytes(20).toString('hex');
  updateUser(id, { apiKey, apiKeyCreatedAt: new Date().toISOString() });
  return apiKey;
}

export function revokeApiKey(id) {
  return updateUser(id, { apiKey: '', apiKeyCreatedAt: '' });
}
