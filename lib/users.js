...
// Dipanggil pas login/daftar via Google. referredBy (opsional) cuma dipasang kalau BENERAN bikin
// akun baru di bagian bawah -- 2 cabang di atas (akun udah pernah connect Google, atau email-nya
// udah kedaftar duluan lewat form biasa) sengaja return lebih awal & gak nyentuh referredBy sama
// sekali, biar konsisten sama aturan "referredBy cuma diisi SEKALI pas akun itu baru kebentuk"
// (lihat lib/referrals.js) -- gak bisa keubah lewat login Google belakangan buat akun yang udah ada.
export function findOrCreateGoogleUser({ googleId, email, name, picture, referredBy = '' }) {
...
// Kode referral: 6 karakter acak, huruf besar + angka -- huruf/angka yang gampang keketuker
// (I/O/0/1) SENGAJA dibuang dari daftar karakternya, soalnya kode ini akan diketik ULANG MANUAL
// sama orang lain (dibagikan lewat caption/chat/dll), bukan cuma di-copy-paste dari link.
...
export function findUserByReferralCode(code) {
  ...
  return getAllUsers().find(u => u.referralCode === clean) || null;
}

// Jumlah akun yang BENERAN terhubung ke referrer ini (referredBy == referrerId), dihitung
// LANGSUNG dari data user -- BUKAN dari referral-log (lihat lib/referrals.js). Referral-log cuma
// kebentuk kalau orang yang direferensiin sudah SUKSES transaksi (buat hitung komisi), jadi kalau
// dipakai buat "Orang Terundang" di halaman Profil, orang yang sudah kepasang kode undangannya
// tapi belum pernah belanja bakal ke-skip / kelihatan kayak belum "terundang" padahal sebenarnya
// udah terhubung -- makanya dipisah, biar statistik ini akurat dari saat kode dipasang, bukan
// nunggu transaksi pertama.
export function countReferredUsers(referrerId) {
  return getAllUsers().filter(u => u.referredBy === referrerId).length;
}

// Akun yang kedaftar SEBELUM fitur referral ini ada belum punya referralCode (field itu cuma
// keisi otomatis di createUser buat akun BARU). Dipanggil pas user buka halaman referral-nya
// sendiri -- generate & simpen kode-nya SEKALI di sini kalau ternyata belum ada, biar akun lama
// tetap kebagian kode tanpa perlu migrasi data manual.
...
export function createUser({ username, email, password, role = 'user', referredBy = '' }) {
  const users = getAllUsers();
  ...
  const user = {
    ...
    // ID user yang kode referral-nya dipakai pas daftar (opsional, kosong kalau gak diisi).
    // Divalidasi di routes/auth.js SEBELUM sampai sini -- createUser cuma nyimpen id-nya mentah2,
    // gak validasi ulang, biar lib ini gak perlu tau soal pesan error yang ditampilkan ke user.
    referredBy: referredBy || '',
    createdAt: new Date().toISOString()
  };
  users.push(user);
